 ...
 */
class GroupUserAssoc extends CActiveRecord
{
	/**
	...
	public static function saveIfNew($group_id, $user_id) {
		Yii::trace("$group_id - $user_id", 'assoc');
		$assoc = new GroupUserAssoc;
		$assoc->group_id = $group_id;
		$assoc->user_id = $user_id;
		if(!$assoc->search()->totalItemCount) {
			$assoc->save();
		}
	}
}